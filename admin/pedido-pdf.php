...
$stmt = db()->prepare('SELECT p.*, c.razao_social, c.cnpj, c.cpf, c.cidade, c.estado, cv.canal AS canal_venda
    FROM pedidos p
...
<head>
    ...
    <title>Pedido <?= e($pedido['numero_pedido']) ?> — <?= date('d/m/Y', strtotime($data_pedido)) ?></title>
...
<div class="action-bar mt-3">
    <a href="<?= BASE_URL ?>/admin/pedido.php?id=<?= $pedidoId ?>" class="btn btn-outline-secondary btn-sm">
        <i class="bi bi-arrow-left me-1"></i>Voltar ao Pedido
    </a>
    ...
    <div class="pdf-body">
        ...
        <div class="info-grid" style="margin-bottom:24px">
            <div class="info-box">
                ...
                <span><?= e($pedido['razao_social'] ?? '—') ?></span>
            </div>
            <div class="info-box">
                <label>CNPJ / CPF</label>
                <span><?= e($pedido['cnpj'] ?? $pedido['cpf'] ?? '—') ?></span>
            </div>
            <div class="info-box">
                <label>Cidade / Estado</label>
                <span><?= e(($pedido['cidade'] ?? '') . ($pedido['estado'] ? ' — ' . $pedido['estado'] : '')) ?></span>
            </div>
            <div class="info-box">
                <label>Canal de Venda</label>
                <span><?= e($pedido['canal_venda'] ?? '—') ?></span>
            </div>
        </div>
