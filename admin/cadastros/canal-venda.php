...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $a = $_POST['action'] ?? '';
    try {
        $d = [$_POST['canal'], $_POST['faixa_faturamento'], (float)$_POST['desconto']];
        if ($a === 'criar') {
            db()->prepare('INSERT INTO canal_venda (canal,faixa_faturamento,desconto) VALUES (?,?,?)')->execute($d);
            flash('success', 'Canal criado!');
        } elseif ($a === 'editar') {
            $d[] = (int)$_POST['id'];
            db()->prepare('UPDATE canal_venda SET canal=?,faixa_faturamento=?,desconto=? WHERE id=?')->execute($d);
...
<table class="table table-hover mb-0">
    <thead class="table-light"><tr><th>Canal</th><th>Faixa de Faturamento</th><th>Desconto Máximo</th><th>Ações</th></tr></thead>
    <tbody>
    ...
    <?php endforeach; else: ?>
        <tr><td colspan="4" class="text-center text-muted py-4">Nenhum canal cadastrado.</td></tr>
    <?php endif; ?>
    </tbody>
</table>
</div></div></div>
...
<script>
function novoReg(){document.getElementById('mt').textContent='Novo Canal';document.getElementById('fa').value='criar';document.getElementById('fi').value='';document.getElementById('f_canal').value='';document.getElementById('f_faixa').value='';document.getElementById('f_desc').value=0;}
function editarReg(d){document.getElementById('mt').textContent='Editar Canal';document.getElementById('fa').value='editar';document.getElementById('fi').value=d.id;document.getElementById('f_canal').value=d.canal||'';document.getElementById('f_faixa').value=d.faixa_faturamento||'';document.getElementById('f_desc').value=d.desconto||0;new bootstrap.Modal(document.getElementById('modal')).show();}
</script>
