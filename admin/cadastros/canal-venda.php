...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $a = $_POST['action'] ?? '';
    try {
        $networkTipo = in_array($_POST['network_tipo'] ?? '', ['Network', 'Network / Accademia'], true) ? $_POST['network_tipo'] : 'Network';
        $d = [$_POST['canal'], $_POST['faixa_faturamento'], (float)$_POST['desconto'], max(0, (float)($_POST['margem_negociacao'] ?? 0)), $networkTipo];
        if ($a === 'criar') {
            db()->prepare('INSERT INTO canal_venda (canal,faixa_faturamento,desconto,margem_negociacao,network_tipo) VALUES (?,?,?,?,?)')->execute($d);
            flash('success', 'Canal criado!');
        } elseif ($a === 'editar') {
            $d[] = (int)$_POST['id'];
            db()->prepare('UPDATE canal_venda SET canal=?,faixa_faturamento=?,desconto=?,margem_negociacao=?,network_tipo=? WHERE id=?')->execute($d);
...
<table class="table table-hover mb-0">
    ...
    <?php if ($canais): foreach ($canais as $c): ?>
        <tr>
            ...
            <td><span class="badge bg-secondary"><?= e($c['network_tipo'] ?? 'Network') ?></span></td>
            <td>
    ...
    <?php endforeach; else: ?>
        <tr><td colspan="6" class="text-center text-muted py-4">Nenhum canal cadastrado.</td></tr>
    <?php endif; ?>
    </tbody>
</table>
</div></div></div>
