...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header('Location: ' . BASE_URL . '/admin/financeiro/contas-receber.php'); exit;
}

$filtro = $_GET['situacao'] ?? '';
$where = $filtro ? 'WHERE cr.situacao=?' : '';
$params = $filtro ? [$filtro] : [];
$titulos = db()->prepare("SELECT cr.*, c.razao_social FROM contas_receber cr LEFT JOIN clientes c ON c.id=cr.cliente_id $where ORDER BY cr.data_vencimento ASC");
$titulos->execute($params);
$titulos = $titulos->fetchAll();

$resumo = db()->query('SELECT
    COALESCE(SUM(CASE WHEN situacao="aberto" THEN valor_receber-descontos END),0) AS aberto,
    COALESCE(SUM(CASE WHEN situacao="vencido" THEN valor_receber-descontos END),0) AS vencido,
    COALESCE(SUM(CASE WHEN situacao="pago" THEN valor_receber-descontos END),0) AS pago
FROM contas_receber')->fetch();
...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modal" onclick="novoReg()"><i class="bi bi-plus-lg me-1"></i>Novo</button>
</div>
...
<div class="modal fade" id="modal" tabindex="-1"><div class="modal-dialog modal-lg modal-dialog-scrollable"><div class="modal-content">
    <form method="POST">
        ...
        <div class="modal-body"><div class="row g-3">
            ...
            <div class="col-md-8"><label class="form-label fw-semibold">Cliente</label><select name="cliente_id" id="f_cli" class="form-select"><option value="">Selecione...</option><?php foreach($clientes as $c):?><option value="<?=$c['id']?>"><?=e($c['razao_social'])?></option><?php endforeach;?></select></div>
