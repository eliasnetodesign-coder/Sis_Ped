    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header text-white border-0" style="background:linear-gradient(135deg,#004733,#2b6a4d)">
                <h5 class="modal-title fw-bold"><i class="bi bi-shield-lock me-2"></i>Defina sua nova senha</h5>
            </div>
            <form method="POST" action="<?= BASE_URL ?>/trocar-senha.php" id="formTrocarSenhaObrig">
                <div class="modal-body">
                    <p class="text-muted small mb-3">
                        <i class="bi bi-info-circle me-1"></i>
                        Por segurança, no primeiro acesso é necessário alterar sua senha antes de continuar.
                    </p>
                    <div class="mb-3">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('formTrocarSenhaObrig').addEventListener('submit', function (e) {
        ...
        if (nova.length < 4)      { e.preventDefault(); erro.textContent = 'A senha deve ter pelo menos 4 caracteres.'; erro.style.display = ''; return; }
        if (nova !== conf)        { e.preventDefault(); erro.textContent = 'As senhas não conferem.'; erro.style.display = ''; return; }
        if (nova === '123')       { e.preventDefault(); erro.textContent = 'Escolha uma senha diferente da senha padrão.'; erro.style.display = ''; return; }
    });
});
</script>
<?php endif; ?>
