...
- `requireAdmin()` — permite `comercial`, `financeiro`, `vendedor`
- `requireComercial()` — permite `comercial`, `vendedor`
...
**Exportar Excel:**
- Gera `.xlsx` com colunas: Código, Razão Social, CNPJ, CPF, Canal de Venda, Desconto Cliente %, Desconto Canal %, Limite de Crédito, E-mail, Telefone 1, Telefone 2, Vendedor, CEP, Endereço, Número, Complemento, Bairro, Cidade, Estado, País, Status.
...
**Operações:** Criar / Editar / Excluir; Preço Padrão criado/atualizado junto.
...
- Associa produto a **três faixas de preço**: Preço Padrão*, Preço Network (opcional), Preço Auxiliar (opcional).
...
- Filtros: Trimestre (1–4) e Ano; padrão = trimestre atual.
- Tabela: código, cliente, vendedor, canal, faturamento no trimestre (pedidos faturados), meta, atingimento %, último log.
- Ações: **Aprovar** ou **Reprovar** por cliente/trimestre/ano — registradas em `bonus_desempenho_logs`.
- Log exibe: status (aprovado/reprovado), nome do usuário gestor, data/hora.
...
- Colunas: código, cliente, vendedor, canal, desconto do canal, faturamento do mês, % MA, valor do bônus MA, **média de atraso** (dias médios de atraso nos pagamentos — semáforo: verde ≤10d, amarelo ≤30d, vermelho >30d), ação, log.
- Cards de resumo: período, clientes elegíveis, total faturado, total bônus MA.
- Ações: **Aprovar** ou **Reprovar** — registradas em `bonus_ma_logs`.
...
- **Workflow de aprovação:** Aprovar ou Reprovar — registrados em `creditos_logs`; último log exibido.
...
**Campos:** Código, Nome*, E-mail*, Senha (obrigatória na criação; em branco = manter), Departamento, Divisão de Vendas, Tipo de Acesso* (`comercial` / `financeiro` / `vendedor`), Tipo de Usuário (texto livre, ex: Gerente, Analista), Telefone, Ramal, Status.
- **Restrição:** não é possível excluir o próprio usuário logado.
- Acesso restrito: `requireAdmin()` (inclui `vendedor`, que pode ver mas normalmente não gerencia usuários).
...
- Acesso: `requireComercial()` (`comercial` + `vendedor`).
- **Etapa 1 — Seleção de Cliente e Produtos:**
  ...
  - Campo de filtro rápido por nome/código do produto (filtra todas as abas em tempo real).
...
- Cards clicáveis por status com qtd e valor (Total Geral, Ag. Comercial, Ag. Financeiro, Ag. Faturamento, Reprovados); card ativo tem borda colorida.
...
- **Ações inline por perfil/status:**
  - `comercial` + status `comercial`: Aprovar → Financeiro, Reprovar.
  - `financeiro` + status `financeiro`: Aprovar → Faturado, Retornar ao Comercial, Reprovar.
...
- Ações (conforme perfil e status atual):
  - `comercial`: Aprovar (→ financeiro) ou Reprovar.
  - `financeiro`: Aprovar (→ faturado), Retornar ao Comercial ou Reprovar.
...
```
[Criado] → comercial → financeiro → faturado
                    ↘ reprovado ↙
...
- Cards por status: comercial, financeiro, faturado, reprovado (qtd + valor total).
- Tabela dos **últimos 20 pedidos** de todos os status: nº pedido, cliente, produto (truncado), vendedor, data, valor, status.
...
- Colunas: Rank, Cliente, Cidade/UF, Vendedor, Pedidos, Valor, Participação %.
...
#### 3.4.7 Faturamento por Vendedor (`admin/relatorios/faturamento-vendedor.php`)
- Filtro: Ano.
- Rankeia vendedores por valor DESC; badge dourado (#1, #2, #3).
- Colunas: Rank, Vendedor, Clientes, Pedidos, Valor, Participação %.
...
- Cards: Total de Pedidos, Aguardando Comercial, Aguardando Financeiro, Faturados, Reprovados, Valor Total Faturado.
...
- Filtro por status (botões: Todos, Aguardando Comercial, Aguardando Financeiro, Aguardando Faturamento, Reprovado).
...
**Campos editáveis:** E-mail (com verificação de unicidade), Telefone 1, Telefone 2, CEP, Endereço, Número, Complemento, Bairro, Cidade, UF, País.
**Campos somente leitura (sidebar):** Código do Cliente, Razão Social, CNPJ, Vendedor, Canal de Venda, Status. Mensagem: "Para alterar dados cadastrais, entre em contato com o suporte."
...
- Trimestral; aprovado ou reprovado manualmente por gestor.
...
- Mensal; aprovado ou reprovado manualmente.
...
| `pedidos` | Pedidos (1 reg. por item) | id, numero_pedido (UNIQUE), tipo_venda, data_pedido, cliente_id (FK), produto_id (FK), vendedor, lote_id, quantidade_total, valor_total, desconto_campanha, forma_pagamento, credito_utilizado, status, observacoes |
